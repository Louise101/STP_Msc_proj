...
def load_real_stage_waits() -> dict[str, pd.Series]:
    stage_files = {
        "ref_to_mri": ("pros_ref_to_mri.csv", "Date of referral to pathway", "Date of MRI"),
        "mri_to_report": ("pros_mri_to_mriclin.csv", "Date of MRI", "Date of clinic"),
        "report_to_biopmdt": ("pros_mriclin_mri_dec.csv", "Date of clinic", "Date of clinic_dec"),
        "biopmdt_to_biopsy": ("pros_mriclin_to_biop.csv", "Date of clinic", "Date of biopsy"),
        "biopsy_to_pathrep": ("pros_biop_to_pathrep.csv", "Date of biopsy", "Date of pathology report"),
        "pathrep_to_treatmdt": ("pros_pathrep_to_treatmdt.csv", "Date of pathology report", "Date of MDT to discuss treatment options"),
        "treatmdt_to_outpat": ("pros_treatmdt_to_outpat.csv", "Date of MDT to discuss treatment options", "Date of OPD appt"),
    }
    ...
    for stage_name, (filename, start_col, end_col) in stage_files.items():
        df = pd.read_csv(
            DATA_DIR / filename,
            usecols=[0, 1, 2],
            names=["Subject number", start_col, end_col],
            header=0,
        ).copy()

        df[start_col] = pd.to_datetime(df[start_col], format="%m/%d/%y", errors="coerce")
        ...
        print(
            ...
            f"min={out[stage_name].min() if len(out[stage_name]) else 'NA'}, "
            f"max={out[stage_name].max() if len(out[stage_name]) else 'NA'}"
        )
...
# Simulation helpers
# --------------------------------------------------
def run_prostad(n_days: int = 365, lam_per_workday: float = 0.586, seed: int = 1234):
...
def derive_sim_stage_waits(sim_events_df: pd.DataFrame) -> dict[str, pd.Series]:
    ...
    if {"date_mri_performed", "date_referral_received"}.issubset(df.columns):
        waits["ref_to_mri"] = diff("date_mri_performed", "date_referral_received")
...
def compare_stage_waits(real_waits: dict[str, pd.Series], sim_waits: dict[str, pd.Series]) -> pd.DataFrame:
    ...
    all_stages = sorted(set(real_waits.keys()) | set(sim_waits.keys()))

    for stage_name in all_stages:
        obs = real_waits.get(stage_name, pd.Series(dtype=float))
        sim = sim_waits.get(stage_name, pd.Series(dtype=float))

        rows.append(summarise_series(stage_name, obs, sim))
...
# --------------------------------------------------
def main():
    print("Running PROSTAD against real PROSTAD data...")

    real_waits = load_real_stage_waits()

    result = run_prostad(n_days=365, lam_per_workday=0.586, seed=1234)
    ...
    print("\n=== Stage wait comparison: real vs PROSTAD sim ===")
    print(stage_comparison.to_string(index=False))

    stage_comparison.to_csv(OUTPUT_DIR / "pros_stage_wait_validation.csv", index=False)

    sim_classified = classify_sim_patients(sim_events_df)
    sim_total_summary = summarise_total_pathway_by_group(sim_classified)

    print("\n=== Simulated total pathway summary by subgroup (PROSTAD) ===")
    print(sim_total_summary.to_string(index=False))

    sim_total_summary.to_csv(OUTPUT_DIR / "pros_total_pathway_summary.csv", index=False)
