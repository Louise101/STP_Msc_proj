...
class CombinedEngineConfig:
    """Top-level configuration for the combined pathway engine.

    The combined engine can represent pure baseline, a mixed baseline/PROSTAD
    stream, or all-PROSTAD by varying only ``p_prostad`` and the two pathway
    rule sets.
    """
...
def sample_patient_pathway(patient: PatientState, base_seed: int, p_prostad: float) -> str:
    """Assign a patient to BASELINE or PROSTAD in a reproducible way."""
...
def get_patient_stage_rules(
    patient: PatientState,
    cfg: CombinedEngineConfig,
) -> tuple[dict[str, str], dict[str, str], dict[str, int]]:
    """Return the active rule set for a specific patient."""
...
def complete_wait_stage_combined(
    ...
    if stage_name == "report_to_biopmdt":
        ...
        patient.data["biopmdt_outcome"] = mdt_outcome
...
def process_all_mc_due_today_until_stable_combined(
    ...
    """Release all MC waits due today, including same-day downstream cascades."""
    total_released = 0
    while True:
...
def run_combined_engine(
    cfg: CombinedEngineConfig,
    *,
    rng: Optional[np.random.Generator] = None,
    daily_referrals_override: dict[date, int] | None = None,
) -> dict[str, Any]:
    """Run the combined engine for one full simulation horizon."""
    ...
    for _ in range(cfg.n_days):
        ...
        for _ in range(n_new):
            patient = create_new_patient(next_patient_id, current_date)
    ...
    for patient in completed_patients:
        patient_results.append((patient.events, (patient.current_date - patient.start_date).days))

    all_patient_results = []
    for patient in all_patients:
        all_patient_results.append((patient.events, (patient.current_date - patient.start_date).days))
    ...
    summary_stats = {
        ...
        "capacity_by_resource": {"MRI_PROSTAD": cfg.mri_capacity_by_weekday_prostad},
        "final_queue_length_by_resource": {"MRI_PROSTAD": mri_resource_prostad.queue_length()},
    }
